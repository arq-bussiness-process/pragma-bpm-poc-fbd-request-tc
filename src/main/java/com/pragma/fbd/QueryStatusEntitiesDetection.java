...
@Service
public class QueryStatusEntitiesDetection implements JavaDelegate{
	...
	RestTemplate MDSrestTemplate;
	 ...
	 //Url consumo cuenta pragma, descomente si se usa dicha cuenta y comente linea anterior
	 String urlBackend = "https://rhwtwpdd05.execute-api.us-east-1.amazonaws.com/dev/customers/documnents/validations/jobs/status";
	 //Url consumo cuenta victor, descomente si se usa dicha cuenta y comente linea anterior
	 //String urlBackend = "https://maky83428e.execute-api.us-east-1.amazonaws.com/dev/customers/documnents/validations/jobs/status";
	 String urlLoadFile = "";
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		//construye url
		UriComponentsBuilder builderUrl = UriComponentsBuilder.fromHttpUrl(urlBackend).queryParam("JobId", JobId);

		//Se arma request
		HttpEntity<String> requestBackend = new HttpEntity<String>(headers);
		
		//Se consume servicio.
		ResponseEntity<QueryJobRs> responseBackend = MDSrestTemplate.exchange(builderUrl.toUriString(), HttpMethod.GET, requestBackend, QueryJobRs.class);
				
		System.out.println("Job: " + JobId);
		System.out.println("estado: " + responseBackend.getBody().getStatusJobId().toString());
		execution.setVariable("JobIdStatus", responseBackend.getBody().getStatusJobId().toString());
	}
}
